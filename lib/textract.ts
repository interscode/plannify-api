...
async function main(imageName: string): Promise<StructuredRow[]> {
    try {
        // Cargar imagen y preparar cliente de Textract
        const client = new TextractClient({ region: "us-east-1" });
        const imagePath = path.resolve(imageName);
        ...
        if (!dayHeaderRow)
            throw new Error("No se encontró la fila con los días");
        ...
        const contentHeaderRow = table.find(
            (row) =>
                row.includes("NOMBRE DE LA MATERIA") && row.includes("PROFESOR")
        );
        if (!contentHeaderRow)
            throw new Error(
                "No se encontró la fila con encabezados de contenido"
            );
        ...
        const startRow =
            Math.max(
                table.indexOf(contentHeaderRow),
                table.indexOf(dayHeaderRow)
            ) + 1;
...
// 👇 Quitar esta línea cuando se integre con la API
main("");
